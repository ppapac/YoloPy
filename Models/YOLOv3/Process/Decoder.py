...
def decode_raw_output(network_output_box: torch.Tensor, i: int):
    ...
    output_shape = network_output_box.size()  # [batch_size, width, height, 51]
    ...
    predict_real_bbox = torch.cat((predict_real_xy, predict_real_wh), axis=4)
    ...
    return torch.concat(
        [predict_real_bbox, predict_confidences, predict_probabilities], axis=4
    )
